...
The full Finance-relevant surface from the CSDL is ~2,500 entities. Generating all of them produces a single module the Ballerina compiler cannot type-check within reasonable JVM heap. The 300-entity cap is a compile-survivable subset that still covers the common integration surface.
