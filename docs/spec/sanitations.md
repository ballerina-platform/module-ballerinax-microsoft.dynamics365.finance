...
- **~300 entity sets** from the D365 F&O Finance domain, each with `list` / `get-by-key` / `create` / `update` / `delete` operations.
...
- **Composite-key paths** (`/CustomersV3(dataAreaId='USMF',CustomerAccount='US-001')`) using PascalCase OData property names in the URL and camelCase parameter names in the generated client (so they don't shadow PascalCase type names).

## Why ~300 and not the full ~2,500

The full Finance-relevant surface from the CSDL is ~2,500 entities. Generating all of them produces ~16,000 public types and ~5,000 remote functions in a single module, which the Ballerina compiler cannot type-check within reasonable JVM heap. The 300-entity cap is a compile-survivable subset that still covers the common integration surface.

If you need an entity that isn't in the spec, add it to the priority list in `docs/spec/tooling/edmx2oas.py` (the `FIN_PRIORITY` tuple) and regenerate.

## How the split between Finance and SCM was decided

Each EntityType in the CSDL is classified by the dominant **label-file prefix** (e.g. `@Ledger`, `@Invent`) across its property `LabelId` annotations. Entities with Finance-side labels go to this connector; SCM-side labels go to the sibling `ballerinax/microsoft.dynamics365.scm`; entities using both (e.g. customers, vendors, products) are `shared` and appear in both connectors. HR, Retail, Commerce, and Enterprise Asset Management entities are excluded entirely. Full rule and tunable sets are in the tooling script.
