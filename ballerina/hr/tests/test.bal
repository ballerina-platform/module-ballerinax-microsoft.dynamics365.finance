...
import ballerinax/microsoft.dynamics365.finance.hr.mock as mockSrv;
...
@test:Config
function testListEmployments() returns error? {
    Client cl = check buildClient();
    EmploymentsCollection response = check cl->listEmployments();
    Employment[] rows = <Employment[]>response.value;
