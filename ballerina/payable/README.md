...
The `microsoft.dynamics365.finance.payable` connector provides APIs for accounts-payable operations in Microsoft Dynamics 365 Finance. It covers vendor master data, vendor groups, vendor payment journals, purchase agreements, delivery terms, and invoice declarations.

### Key Features

- Create, read, update, and delete vendor master records (V2 and V3)
- Vendor groups and vendor payment journal headers and lines
- Purchase agreements and pay-when-paid text configuration
- Delivery terms and intent-letter setup
- Vendor invoice declaration management
...
To use the `microsoft.dynamics365.finance.payable` connector in your Ballerina application, modify the `.bal` file as follows:

### Step 1: Import the module

```ballerina
import ballerinax/microsoft.dynamics365.finance.payable as finPayable;
```
...
finPayable:Client finPayableClient = check new (
    config = {
        auth: {
...
```ballerina
finPayable:VendorsV3Collection vendors = check finPayableClient->listVendorsV3();
```
