...
import ballerinax/microsoft.dynamics365.finance.budget.mock as mockSrv;
...
@test:Config
function testListBudgetCodes() returns error? {
    Client cl = check buildClient();
    BudgetCodesCollection response = check cl->listBudgetCodes();
