...
import ballerinax/microsoft.dynamics365.finance.project.mock as mockSrv;
...
@test:Config
function testListCDSProjects() returns error? {
    Client cl = check buildClient();
    CDSProjectsCollection response = check cl->listCDSProjects();
