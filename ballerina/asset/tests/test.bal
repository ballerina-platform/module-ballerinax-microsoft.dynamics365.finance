...
import ballerinax/microsoft.dynamics365.finance.asset.mock as mockSrv;
...
@test:Config
function testListFixedAssets() returns error? {
    Client cl = check buildClient();
    FixedAssetsCollection response = check cl->listFixedAssets();
    FixedAsset[] rows = <FixedAsset[]>response.value;
