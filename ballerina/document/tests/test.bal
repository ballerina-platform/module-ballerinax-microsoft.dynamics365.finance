...
import ballerinax/microsoft.dynamics365.finance.document.mock as mockSrv;
...
@test:Config
function testListAgentFeeds() returns error? {
    Client cl = check buildClient();
    AgentFeedsCollection response = check cl->listAgentFeeds();
